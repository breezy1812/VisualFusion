#!/usr/bin/env python3
"""
PyTorch to TensorRT Conversion Script (FP32→FP32→FP16 流程)
專門實現：PyTorch(FP32) → ONNX(FP32) → TensorRT(FP16) 轉換流程

轉換流程：
- PyTorch 模型：始終保持 FP32 精度以確保數值穩定性
- ONNX 模型：始終保持 FP32 精度以確保相容性  
- TensorRT 引擎：只在最後階段使用 FP16 精度以提升推理速度
...
Usage:
    python export_py32onnx32TRT16.py --fp16  # TensorRT 使用 FP16，PyTorch/ONNX 保持 FP32
    python export_py32onnx32TRT16.py         # 全程使用 FP32 模式
    python export_py32onnx32TRT16.py --fp16 --opset 12
"""
