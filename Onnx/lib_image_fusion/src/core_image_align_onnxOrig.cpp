...
// check input_data
            if(!current_image_name_.empty()){
                std::string input_csv_filename = "/circ330/forgithub/VisualFusion_libtorch/Onnx/output/input_data_"+current_image_name_ + ".csv";
                ...
                // 完全模擬LibTorch的tensor存取方式以確保數值完全一致
                // 創建 tensor-like 存取函數，模擬 eo_fp32[0][0][i][j] 的行為
                auto get_tensor_value = [](const std::vector<float>& data, int height, int width, int i, int j) -> float {
                    return data[i * width + j];  // 行優先索引，完全對應[0][0][i][j]
                };
                
                // 使用與LibTorch完全相同的存取模式：eo_fp32.size(2) 和 eo_fp32.size(3)
                for (int i = 0; i < param_.pred_height; ++i) {        // 對應 LibTorch 的 eo_fp32.size(2)
                    for (int j = 0; j < param_.pred_width; ++j) {     // 對應 LibTorch 的 eo_fp32.size(3)
